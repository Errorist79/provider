...
WORKSPACE_RESPONSE=$(curl -sf -X POST "$UNKEY_API/api/v1/workspaces" \
...
if [ -z "$WORKSPACE_ID" ]; then
    ...
    WORKSPACE_ID=$(curl -sf "$UNKEY_API/api/v1/workspaces" \
...
API_RESPONSE=$(curl -sf -X POST "$UNKEY_API/api/v1/apis" \
    -H "Authorization: Bearer $ROOT_KEY" \
    -H "Content-Type: application/json" \
    -d "{
        \"name\": \"RPC Keys\",
...
if [ -z "$API_ID" ]; then
    ...
    API_ID=$(curl -sf "$UNKEY_API/api/v1/apis?workspaceId=$WORKSPACE_ID" \
...
TEST_KEY_RESPONSE=$(curl -sf -X POST "$UNKEY_API/api/v1/keys" \
    -H "Authorization: Bearer $ROOT_KEY" \
    -H "Content-Type: application/json" \
    -d "{
        \"apiId\": \"$API_ID\",
